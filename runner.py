...
def run_agentic(user_text: str, llm_api_base: str, llm_api_key: str, llm_model: str, openapi_base: str, max_rounds: int = 3):
    ...
    for round_idx in range(1, max_rounds + 1):
        ...
        # No quote yet -> append tool results and continue to next round
        messages.append({"role": "assistant", "content": raw})
        messages.append({
            "role": "user",
            "content": f"TOOL_RESULTS (trace_id={trace_id}) round={round_idx}:\n{json.dumps(tool_results_this_round, ensure_ascii=False)}\n\nContinue and produce get_shipping_quote JSON objects.",
        })
